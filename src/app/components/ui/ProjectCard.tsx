...
export function ProjectCard({ project }: { project: TProject }) {
  const { thumbnail, title, description, _id, techs, url, github_server } =
    project || {};
  return (
    <Card>
      <div className="relative">
        <Link href={`/projects/${_id}`}>
          <Image
            ...
            alt={title}
          />
          <div className="absolute bg-black/5 inset-0 "></div>
        </Link>
      </div>
      <CardTitle
...
const Skeleton = ({ techs }: { techs: string[] }) => {
  ...
  return (
    <div
      ...
      <div className="h-20 w-px absolute top-30 m-auto z-40 bg-gradient-to-b from-transparent via-cyan-500 to-transparent animate-move">
...
export const Card = ({
  ...
  return (
    <div
      className={cn(
        "min-w-sm w-full mx-auto p-6 rounded-xl border border-[rgba(255,255,255,0.10)] dark:bg-[rgba(40,40,40,0.70)] bg-gray-100 shadow-[2px_4px_16px_0px_rgba(248,248,248,0.06)_inset] group relative",
        className
      )}>
      {children}
    </div>
...
export const CardTitle = ({
  ...
  return (
    <div className="flex items-center justify-between gap-3">
      ...
      <a href={url === "#" ? github_server : url} target="_blank">
        <span className="inline-block text-white-100 font-medium text-sm">
          {url === "#" ? "Github" : "Live Demo"}
        </span>
        <FaLocationArrow
          size={10}
          className="inline-block text-white-100 ml-[6px]"
        />
      </a>
    </div>
  );
...
export const CardSkeletonContainer = ({
  ...
  return (
    <div
      className={cn(
        "h-[150px] rounded-xl z-40 absolute -bottom-5 left-0 w-full",
        className,
        showGradient &&
          "bg-neutral-300 dark:bg-[rgba(40,40,40,0.34)] [mask-image:radial-gradient(50%_50%_at_50%_50%,white_50%,transparent_100%)]"
      )}>
      {children}
    </div>
  );
